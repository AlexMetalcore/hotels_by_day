...
try {
    ...
    foreach ($data as $item) {
        ...
        echo 'Get the Person based on ID: name - ' . $person->getName() . '; surname - ' . $person->getSurname() . '; gender - ' . $person->getSex() . PHP_EOL;
    }
    ...
    echo 'Time usage ' . round($time, 2) . ' s' . PHP_EOL;
    echo 'Memory usage ' . round($mem_usage / (1024 * 1024)) . ' Mb' . PHP_EOL;
    echo 'Max memory usage ' . round($mem_peak / (1024 * 1024)) . ' Mb' . PHP_EOL;
